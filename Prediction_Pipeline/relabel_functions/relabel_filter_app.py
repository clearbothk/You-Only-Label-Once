...
# Variables -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
    ...
    def copy_files():
        """
        Deletes any existing relabel_Object Materials folder before creating and copying in cropped images according the materials dictionary
        """
        try:
            global stats_dict
            global material_dict
            ...
            for i in material_dict:
                if f'{object_class}_{i}' in os.listdir(f'{prediction_folder}/relabel_Object Materials'):
                    shutil.rmtree(f'{prediction_folder}/relabel_Object Materials/{object_class}_{i}')
                    print(f"{i}'s original directory has be deleted")
                if len(material_dict[i]) != 0:
                    os.makedirs(f'{prediction_folder}/relabel_Object Materials/{object_class}_{i}',exist_ok=True)
                    print(f"{i}'s has been created")
                    for file in material_dict[i]:
                        shutil.copy(file,f'{prediction_folder}/relabel_Object Materials/{object_class}_{i}')
        except NameError:
