...
def correct_check(images, labels, window):
    ...
    def save_dict():
        global correct_dict
        with open(f'{image640_path}/correct_dict.json' , 'w') as f:
            json.dump(correct_dict,f)

    def load_dict():
        global correct_dict
        with open(f'{image640_path}/correct_dict.json' , 'r') as f:
    ...
    def copy_files():
        ...
        for file in correct_dict["Correct"]:
            try:
                ...
                shutil.copy(f'{Project}/fullsize_images/{file_name}', f'{Pred}/relabel_Correct/images/{file_name}')
                shutil.copy(f'{IC}/labels/{file_name[:-4]}.txt', f'{Pred}/relabel_Correct/labels/{file_name[:-4]}.txt')
                print('CORRECT', f'{file_name} copied')
            except FileNotFoundError:
                print('CORRECT', f'Label for {file_name} not found!')
        for file in correct_dict["Incorrect"]:
            try:
                file_name = file.split("/")[-1]
                shutil.copy(f'{Project}/fullsize_images/{file_name}', f'{Pred}/relabel_Incorrect/images/{file_name}')
                shutil.copy(f'{IC}/labels/{file_name[:-4]}.txt', f'{Pred}/relabel_Incorrect/labels/{file_name[:-4]}.txt')
                print('INCORRECT', f'{file_name} copied')
            except FileNotFoundError:
                print('INCORRECT', f'Label for {file_name} not found!')
        for file in correct_dict["Remove"]:
            print('REMOVE')
            try:
                file_name = file.split("/")[-1]
                shutil.copy(f'{Project}/fullsize_images/{file_name}', f'{Pred}/relabel_Remove/images/{file_name}')
                shutil.copy(f'{IC}/labels/{file_name[:-4]}.txt', f'{Pred}/relabel_Remove/labels/{file_name[:-4]}.txt')
                print('REMOVE', f'{file_name} copied')
            except FileNotFoundError:
                print('REMOVE', f'Label for {file_name} not found!')
        root.update()
        root.quit()
        root.destroy()
